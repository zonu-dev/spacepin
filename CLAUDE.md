...
- `bash scripts/build_release_dmg.sh` - Developer ID 署名済みの直配布用 DMG を作成
- `bash scripts/notarize_release_dmg.sh` - 直配布 app / DMG を公証して stapling する
...
- 公証は `xcrun notarytool store-credentials ...` で Keychain profile を作ってから `SPACEPIN_NOTARY_KEYCHAIN_PROFILE=<profile> bash scripts/notarize_release_dmg.sh`
